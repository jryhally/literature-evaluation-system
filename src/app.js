...
const nonAuthorSignals = ["奖", "出版社", "文学奖", "电视剧", "电影", "杂志", "期刊", "出版社"];
...
async function searchAuthorWikipedia(authorName) {
  ...
  const results = await Promise.allSettled([
    searchWikipediaCandidates(`${authorName} 作家`, 5),
    searchWikipediaCandidates(authorName, 5)
  ]);
  ...
  const scored = candidates.map((candidate) => {
    ...
    const authorLike = /作家|小说家|詩人|诗人|剧作家|文学家|散文家/.test(haystack);
    const nonAuthorLike = nonAuthorSignals.some((signal) => candidate.title.includes(signal));
    return {
      ...candidate,
      matchScore:
        (normalizedTitle === normalizedAuthor ? 100 : 0) +
        (normalizedTitle.includes(normalizedAuthor) || normalizedAuthor.includes(normalizedTitle) ? 42 : 0) +
        (authorLike ? 34 : 0) -
        (nonAuthorLike ? 80 : 0)
    };
  });
  const best = scored.sort((a, b) => b.matchScore - a.matchScore)[0];
  return best ? fetchWikipediaPage(best.pageid, best) : null;
}
...
function extractWorkTitlesFromAuthor(authorInfo) {
  const text = `${authorInfo?.extract || ""} ${(authorInfo?.categories || []).join(" ")}`;
  const titles = [...text.matchAll(/《([^》]{1,32})》/g)]
    .map((match) => match[1].trim())
    .filter((name) => !/[，。、“”《》]/.test(name));
  return [...new Set(titles)].slice(0, 8);
}

function inferAuthorFromWork(work, requestedAuthor = "") {
  const text = `${work?.title || ""} ${work?.extract || ""} ${(work?.categories || []).join(" ")}`;
  if (requestedAuthor && text.includes(requestedAuthor)) return requestedAuthor;

  const patterns = [
    /(?:作家|小说家)([\u4e00-\u9fa5·]{2,24})的(?:作品|小说|長篇|长篇|代表作)/,
    /(?:作家|小说家)([\u4e00-\u9fa5·]{2,24})的/,
    /作者(?:是|為|为|：|:|\s)*([\u4e00-\u9fa5·]{2,24})(?=，|。|于|於|创作|創作|的|$)/,
    /作者[是為为：:\s]+([\u4e00-\u9fa5·]{2,24})/,
    /由([\u4e00-\u9fa5·]{2,24})(?:创作|創作|所著|撰写|撰寫|著)/,
    /([\u4e00-\u9fa5·]{2,24})(?:创作|創作|所著|撰写|撰寫)/
  ];
  for (const pattern of patterns) {
    const match = text.match(pattern);
    const candidate = match?.[1]?.replace(/[的之所著撰写創作创作]+$/g, "");
    if (candidate && !nonAuthorSignals.some((signal) => candidate.includes(signal))) return candidate;
  }

  const categoryAuthor = (work?.categories || [])
    .filter((category) => !/背景|题材|題材|改编|改編|電影|电影|电视剧|電視劇|时期|時期/.test(category))
    .map((category) => category.match(/^([\u4e00-\u9fa5·]{2,12})(?:小说|作品|短篇小说|长篇小说)$/)?.[1])
    .find(Boolean);
  return categoryAuthor || requestedAuthor || "";
}

async function findWorkCandidates(title, author) {
  const queries = [];
  if (title && author) queries.push(`${title} ${author} 文学作品`, `${title} ${author}`);
  if (title) queries.push(`${title} 小说`, `${title} 文学作品`, title);
  if (author) queries.push(`${author} 代表作 小说`, `${author} 作品 文学`, `${author} 长篇小说`);

  let authorWorkTitles = [];
  if (!title && author) {
    const authorInfo = await searchWikipedia(`${author} 作家`);
    authorWorkTitles = extractWorkTitlesFromAuthor(authorInfo);
    authorWorkTitles.forEach((workTitle) => queries.unshift(`${workTitle} ${author} 小说`));
  }

  const results = await Promise.allSettled(queries.map((query) => searchWikipediaCandidates(query, 5)));
  const candidates = dedupeCandidates(results.flatMap((result) => result.status === "fulfilled" ? result.value : []));
  const candidatePool = candidates.length > 1 && author
    ? candidates.filter((candidate) => normalizeName(candidate.title) !== normalizeName(author))
    : candidates;
  const scored = candidatePool.map((candidate) => {
    const haystack = `${candidate.title} ${candidate.snippet}`;
    const extractedWorkHit = authorWorkTitles.includes(candidate.title);
    const exactTitle = Boolean(title) && candidate.title === title;
    const titleInTitle = Boolean(title) && (candidate.title.includes(title) || title.includes(candidate.title));
    const titleInSnippet = Boolean(title) && candidate.snippet.includes(title);
    const authorHit = author && haystack.includes(author);
    const literaryHit = /小说|文学|作品|长篇|短篇|诗|戏剧|散文|作家|改编/.test(haystack);
    const likelyAuthorPage = author && normalizeName(candidate.title) === normalizeName(author);
    return {
      ...candidate,
      matchScore:
        (extractedWorkHit ? 135 : 0) +
        (exactTitle ? 120 : 0) +
        (!exactTitle && titleInTitle ? 58 : 0) +
        (titleInSnippet ? 24 : 0) +
        (authorHit ? 28 : 0) +
        (literaryHit ? 14 : 0) +
        (likelyAuthorPage ? -80 : 0) +
        (title ? Math.max(0, 6 - Math.abs(candidate.title.length - title.length)) : 0)
    };
  });
...
async function fetchLiteraryContextFromCandidate(candidate, author) {
  const work = await fetchWikipediaPage(candidate.pageid, candidate);
  const inferredAuthor = inferAuthorFromWork(work, author);
  const authorInfo = inferredAuthor ? await searchAuthorWikipedia(inferredAuthor) : null;
...
function buildReasons(contextText, work, authorInfo, requestedAuthor = "", effectiveAuthor = "") {
  const reasons = [];
  ...
  if (requestedAuthor && effectiveAuthor && requestedAuthor !== effectiveAuthor) {
    reasons.push(`注意：你输入的作者是“${requestedAuthor}”，但作品资料显示更可能是“${effectiveAuthor}”。作者板块已按作品资料自动修正。`);
  } else if (requestedAuthor && work?.extract && !`${work.title} ${work.extract}`.includes(requestedAuthor)) {
...
function evaluate(title, author, context) {
  const effectiveAuthor = context.inferredAuthor || author;
  ...
  const contextText = `${title} ${effectiveAuthor} ${workText} ${authorText} ${categoryText}`;
  ...
  return {
    ...
    dimensions: scored,
    confidence,
    reasons: buildReasons(contextText, context.work, context.author, context.requestedAuthor || author, effectiveAuthor),
...
function renderContext(title, author, context) {
  ...
  $("#author-heading").textContent = context.author?.title || context.inferredAuthor || author || "作者背景";
...
async function selectCandidate(index) {
  ...
  try {
    ...
    const effectiveAuthor = context.inferredAuthor || author;
    renderContext(candidate.title, effectiveAuthor, context);
    renderScores(evaluate(candidate.title, effectiveAuthor, context));
...
async function handleEvaluation(event) {
  ...
  const author = $("#author-name").value.trim();
  if (!title && !author) return showToast("作品名和作者至少填一个");
